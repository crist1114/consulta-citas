...
pipeline {
  ...
  post {
    ...
    success {
      ...
      junit 'microservicio/**/test-results/test/*.xml'
    }
    failure {
      echo 'This will run only if failed'
      mail (to: 'cristian.medina@ceiba.com.co',subject: "Failed Pipeline:${currentBuild.fullDisplayName}",body: "Something is wrong with ${env.BUILD_URL}")
    }
    unstable {
